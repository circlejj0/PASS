...
class HoppingCont(Node):
    ...
    def process(self):
        ...
        # 도킹 시작 지점에서 멈춤
        if self.next_obj == len(self.path_points) - 1 and self.distance < self.close_distance:
            self.stop_wamv()
            self.get_logger().info("최종 위치 도달 - WAM-V 정지")
            return

        # 도킹 시작 지점 위치 유지
        if self.Hopping_hold:
            elapsed = (self.get_clock().now() - self.Hopping_start_time).nanoseconds * 1e-9
            if elapsed >= 3.0:
                self.get_logger().info("최종 위치 도달")
                return
            else:
                self.stop_wamv()
                return
