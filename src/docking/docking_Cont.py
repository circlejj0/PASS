...
class DockingCont(Node):
    ...
    def status_callback(self, msg):
        if msg.data == "ARRIVED_P1" and self.mode == "WAYPOINT_FOLLOWING":
            self.mode = "ROTATING"
            self.get_logger().info("MODE CHANGE: WAYPOINT_FOLLOWING -> ROTATING")
            self.stop_wamv()
    ...
    def rotate_in_place(self):
        ...
        if abs(math.degrees(error_yaw)) < 3.0:
            self.get_logger().info("Rotation Complete. -> HOLDING_FOR_SCAN")
    ...
    def dock_with_lidar_and_vision(self):
        ...
        # ==================== [ 수정된 부분: 제어 로직 부호 변경 ] ====================
        # [기존] turn_adjustment = -self.Kp_visual * self.visual_error - self.Kd_visual * error_derivative
        # [변경] turn_adjustment = +self.Kp_visual * self.visual_error + self.Kd_visual * error_derivative
        #       (오른쪽(+) 오차일 때 오른쪽(+)으로 돌도록 부호를 바로잡음)
        ...
        # 오른쪽으로 돌려면 left_thrust가 더 커야 하므로 turn_adjustment를 더해줍니다.
